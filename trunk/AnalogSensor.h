...
typedef struct {
  ...
  int fireThresholdAvgDev;   		// an integer           0 == do not use
...
*/
void initAnalogSensor(analogSensor_t &sensor, char* name, int pin, int sample_count, unsigned long int thresholdPct,
  unsigned long int thresholdDelta, unsigned long int thresholdAvgDev, bool fireThresholdDirectionSensitive,
...
int currentAnalogSensorAverageDeviationReading(analogSensor_t&);
...
public:
  ...
  unsigned long int thresholdAvgDev,
  ...
  int currentAverageDeviationReading();
  int lastDelta();
  bool isFiring();
  void takeSample();
  void takeAllSamples();
